...
ilx.addMethod('Generate-FederationMetadata', function (req, res) {
  var issuervar = req.params()[0]
  var endpointvar = req.params()[1]

    // Metadata Options
    // issuer: Issuer URL
    // endpointPath: path to federationmetadata.xml
    // cert: signing certificate
    // key: signing key
    // digestAlgorith: Will default to sha256
    // signatureAlgorith: Will default to sha256

  var metaoptions = {
    issuer: issuervar,
    endpointPath: endpointvar,
    cert: SigningCert,
    key: SigningKey
  }

  var metadatavar = metadata(metaoptions, issuervar, endpointvar)
    // console.log(metadatavar);
  res.reply(metadatavar)
})
